...
int index_student = 0;
int index_dorm = 0;
int index_left_students = 0;
int index_left_asrama = 0;
int total_students_ = 0;
   ...
   char perintah[60];
    while(1){
        ...
        if (strcmp(token,"student-add")==0){
            ...
            char *_id_ = token;
            for (int i = 0; i < total_students;i++){
                if(strcmp(students[i].id,_id_)==0){
                    index_left_students = i;
                }
            }
            strcpy(LEFT[0].name,"left");
            LEFT[0].capacity = 10;
            LEFT[0].gender = GENDER_FEMALE;
            LEFT[0].residents_num = 0;
            students[index_left_students].dorm = LEFT;
        }
